...
class SleeperPlatform(BasePlatform):
    ...
    def query_with_delayed_refresh(
        self, url: str, save_file: Path, check_for_saved_data: bool = False, refresh_days_delay: int = 1
    ):
    ...
    def _map_player_data_to_matchup(self, matchup, week):
        for team in matchup:
            ...
            if team["starters"] and team["players"]:
                ...
                team["roster"] = [self._fetch_player_data(player_id, week) for player_id in team["players"]]
            else:
                team["roster"] = []
    ...
    def map_data_to_base(self) -> None:
        ...
        self.standings = sorted(
            ...
            key=lambda x: (
                ...
                float(
                    ...
                    + "."
                    + (str(x.get("settings").get("fpts_decimal") if x.get("settings").get("fpts_decimal") else "0"))
                ),
            ),
            reverse=True,
        )
        ...
        for team in self.standings:
            ...
            team["co_owners"] = (
                [league_managers.get(co_owner) for co_owner in team.get("co_owners")] if team.get("co_owners") else []
            )
        ...
        for week, matchups in matchups_by_week.items():
            ...
            for matchup in matchups:
                ...
                base_matchup.tied = (
                    ...
                    and float(matchup[0].get("points")) == float(matchup[1].get("points"))
                    else False
                )
                    ...
                    if not base_matchup.tied:
                        if base_team.team_id == matchup[0].get("roster_id"):
                            ...
                            if (
                                ...
                                and float(matchup[1].get("points")) > float(matchup[0].get("points"))
                            ):
                                base_matchup.winner = base_team
                            else:
                                base_matchup.loser = base_team
                ...
                for player in roster:
                    if player:
                        ...
                        base_player.points, base_player.projected_points = self._get_player_points(
                            stats=player_stats, projected_stats=player.get("projected")
                        )
                        ...
                        base_player.season_points, base_player.season_projected_points = self._get_player_points(
                            ...
                            if str(base_player.player_id) in player_season_stats.keys()
                            else [],
                            projected_stats=player_season_projected_stats[str(base_player.player_id)]
                            if str(base_player.player_id) in player_season_projected_stats.keys()
                            else [],
                        )
                        ...
                        if player["starter"]:
                            ...
                            available_primary_slots = list(
                                base_player.eligible_positions.intersection(set(team_filled_positions)).difference(
                                    set(
                                        [
                                            pos.get("base")
                            ...
                            available_wrrb_flex_slots = list(
                                base_player.eligible_positions.intersection(set(self.league.flex_positions_rb_wr))
                            )

                            available_rec_flex_slots = list(
                                base_player.eligible_positions.intersection(set(self.league.flex_positions_te_wr))
                            )

                            available_flex_slots = list(
                                base_player.eligible_positions.intersection(set(self.league.flex_positions_rb_te_wr))
                            )

                            available_super_flex_slots = list(
                                base_player.eligible_positions.intersection(set(self.league.flex_positions_qb_rb_te_wr))
                            )

                            available_idp_flex_slots = list(
                                base_player.eligible_positions.intersection(set(self.league.flex_positions_idp))
                            )
