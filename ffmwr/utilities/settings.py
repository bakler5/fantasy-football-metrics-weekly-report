...
from ffmwr.utilities.exceptions import AppConfigError
...
class ReportSettings(CustomSettings):
    ...
    show_donation_footer: bool = Field(
        False,
        title=__qualname__,
        description="include donation footer and links in generated PDF report",
    )
    footer_text: str = Field(
        "Generated by Josh Bachler for the Josh Koch Memorial League",
        title=__qualname__,
...
def get_app_settings_from_env_file(env_file_path: Path) -> AppSettings:
    ...
    if env_file_path.is_file():
        if os.access(env_file_path, mode=os.R_OK):
            ...
            message = 'Unable to access ".env" file. Please check that file permissions are properly set.'
            logger.error(message)
            raise AppConfigError(message)
        ...
        if create_env_file == "y":
            ...
            message = 'Local ".env" not found. Please make sure that it exists in project root directory.'
            logger.error(message)
            raise AppConfigError(message)
        else:
...
def create_env_file_from_settings(
    ...
    # Optional customization for report footer text
    customize_footer = input(
        f"{Fore.GREEN}Would you like to customize the PDF footer text? "
        f"{Fore.YELLOW}({Fore.GREEN}y{Fore.YELLOW}/{Fore.RED}n{Fore.YELLOW}) -> {Style.RESET_ALL}"
    ).lower()
    if customize_footer == "y":
        default_footer = app_settings.report_settings.footer_text
        footer_text = input(
            f"{Fore.GREEN}Enter footer text {Fore.WHITE}[default: {default_footer}] -> {Style.RESET_ALL}"
        )
        if footer_text.strip():
            app_settings.report_settings.footer_text = footer_text.strip()

    # Optional donation footer with links/images
    include_donations = input(
        f"{Fore.GREEN}Include donation footer with links and QR codes? "
        f"{Fore.YELLOW}({Fore.GREEN}y{Fore.YELLOW}/{Fore.RED}n{Fore.YELLOW}) -> {Style.RESET_ALL}"
    ).lower()
    if include_donations in ("y", "n"):
        app_settings.report_settings.show_donation_footer = include_donations == "y"
